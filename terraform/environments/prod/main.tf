...
module "ec2" {
  ...
  s3_backup_bucket         = var.enable_backups ? "blindstrader-backups-${var.environment}" : ""
...
module "storage" {
  ...
  ebs_volume_arns = var.enable_backups ? [
    module.ec2.mysql_volume_arn,
    module.ec2.redis_volume_arn,
    module.ec2.monitoring_volume_arn
  ] : []
}
